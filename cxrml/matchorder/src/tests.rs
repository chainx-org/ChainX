...
fn print_bid(pair: OrderPair, order_type: OrderType) {
    ...
    let bids: Vec<
        Bid<
            <tests::Test as pendingorders::Trait>::Amount,
            <tests::Test as pendingorders::Trait>::Price,
        >,
    > = MatchOrder::bid_list((pair, order_type));
    for i in 0..bids.len() {
        println!("---{}---", i);
        println!("price:{:?}", bids[i].price);
        println!("sum:{:?}", bids[i].sum);
        println!("list:{:?}", bids[i].list.len());
        for j in 0..bids[i].list.len() {
            println!("      [{:?}] {:?}", j, bids[i].list[j]);
            let bid_detail = MatchOrder::bid_of(bids[i].list[j]).unwrap();
