...
fn test_register() {
    with_externalities(&mut new_test_ext(), || {
        let t_sym: Symbol = slice_to_u8_8(b"x-eos");
        let t_desc: TokenDesc = slice_to_u8_32(b"eos token");
        let precision = 4;
        ...
        // test err branch
        let btc_t = Token::new(btc_symbol, slice_to_u8_32(b"btc token"), 4);
        assert_noop!(TokenBalances::register_token(btc_t, 0, 0), "already has this token symbol");
        assert_eq!(TokenBalances::token_list_len(), 3);
        assert_eq!(TokenBalances::token_list_map(3), (false, slice_to_u8_8(b"")));
    })
}
...
fn test_remove() {
    with_externalities(&mut new_test_ext(), || {
        ...
        let t_new: TokenT<Test> = Token { symbol: slice_to_u8_8(b"x-eos2"), ..t };
...
fn test_normal_issue_and_destroy() {
    with_externalities(&mut new_test_ext(), || {
        ...
        // issue
        TokenBalances::issue(&a, &btc_symbol, 50).unwrap();
        assert_eq!(TokenBalances::total_token_of(&a, &btc_symbol), 50);
        assert_eq!(TokenBalances::total_token(&btc_symbol), 150);

        // lock
        TokenBalances::lock_destroy_token(&a, &btc_symbol, 25).unwrap();
        ...
        // destroy
        TokenBalances::destroy(&a, &btc_symbol, 25).unwrap();
        assert_eq!(TokenBalances::locked_token_of(&a, &btc_symbol), 0);
        assert_eq!(TokenBalances::free_token_of(&a, &btc_symbol), 25);
...
fn test_unlock_issue_and_destroy2() {
    with_externalities(&mut new_test_ext(), || {
        ...
        // unlock
        TokenBalances::unlock_destroy_token(&a, &btc_symbol, 10).unwrap();
...
fn test_error_issue_and_destroy1() {
    with_externalities(&mut new_test_ext(), || {
        ...
        // destroy first
        // destroy
        assert_err!(TokenBalances::destroy(&a, &btc_symbol, 25), "not enough locked token to destroy");
        // lock
        assert_eq!(TokenBalances::total_free_token(&btc_symbol), 150);
        assert_err!(TokenBalances::lock_destroy_token(&a, &btc_symbol, 100), "not enough free token to lock for this account");
        // lock first
        assert_ok!(TokenBalances::lock_destroy_token(&a, &btc_symbol, 25));
        // destroy
        assert_ok!(TokenBalances::destroy(&a, &btc_symbol, 25));
    })
}
...
fn test_error_issue_and_destroy2() {
    with_externalities(&mut new_test_ext(), || {
        ...
        // overflow
        let i: i32 = -1;
        assert_err!(TokenBalances::lock_destroy_token(&a, &btc_symbol, i as TokenBalance), "not enough free token to lock");
        assert_err!(TokenBalances::issue(&a, &btc_symbol, i as TokenBalance), "Overflow in increase_total_free_token_by");
    })
}
...
fn test_error_issue_and_destroy3() {
    with_externalities(&mut new_test_ext(), || {
        ...
        assert_err!(TokenBalances::destroy(&a, &btc_symbol, 25), "not a existed token in this account token list");
        assert_err!(TokenBalances::lock_destroy_token(&a, &btc_symbol, 25), "not a existed token in this account token list");
        TokenBalances::issue(&a, &btc_symbol, 0).unwrap();
        assert_err!(TokenBalances::destroy(&a, &btc_symbol, 25), "not enough locked token to destroy");
        assert_err!(TokenBalances::lock_destroy_token(&a, &btc_symbol, 25), "not enough free token to lock for this account");
...
fn test_transfer_err() {
    with_externalities(&mut new_test_ext(), || {
        ...
        TokenBalances::issue(&b, &btc_symbol, 50).unwrap();
        // transfer
