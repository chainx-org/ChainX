...
use runtime_primitives::traits::{As, Hash, OnFinalise, SimpleArithmetic, Zero};
use runtime_support::dispatch::Result;
use runtime_support::{StorageMap, StorageValue};

use financialrecords::{OnDepositToken, OnWithdrawToken};
use staking::{OnNewSessionForTokenStaking, OnReward, Validator, VoteWeight};
use tokenbalances::{OnMoveToken, Symbol};
use pendingorders::OrderPair;

pub trait Trait: system::Trait + balances::Trait + tokenbalances::Trait + staking::Trait + pendingorders::Trait {
...
#[derive(PartialEq, Eq, Clone, Encode, Decode, Default)]
#[cfg_attr(feature = "std", derive(Serialize, Deserialize, Debug))]
pub struct HodlingRecord<TokenBalance, BlockNumber>
    where
        TokenBalance: Default + As<u128> + Copy + Clone,
...
impl<TokenBalance, C> VoteWeight<C> for HodlingRecord<TokenBalance, C>
    where
        TokenBalance: Default + As<u128> + Copy + Clone + SimpleArithmetic,
        C: Default + As<u64> + Copy + Clone,
{
    fn amount(&self) -> u128 {
        self.amount.as_()
    }
    ...
    fn last_acum_weight_update(&self) -> u128 {
        self.last_weight_update.clone().as_() as u128
    }
    ...
    fn set_last_acum_weight(&mut self, latest_vote_weight: u128) {
        self.last_weight = latest_vote_weight;
    }

    fn set_last_acum_weight_update(&mut self, current_block: C) {
        self.last_weight_update = current_block;
    }
}
...
decl_storage! {
    trait Store for Module<T: Trait> as TokenStkaing {
        ...
        pub HodlingRecordFor get(hodling_record_for): map (T::AccountId, Symbol) => HodlingRecord<T::TokenBalance, T::BlockNumber>;
...
impl<T: Trait> OnMoveToken<T::AccountId, T::TokenBalance> for Module<T> {
    fn on_move_token(from: &T::AccountId, to: &T::AccountId, sym: &Symbol, value: T::TokenBalance) {
        if is_valid_exchange_token::<T>(sym) == false { return; }
        ...
        let mut from_hodling = Self::hodling_record_for(&key_from);
        let mut to_hodling = Self::hodling_record_for(&key_to);
        ...
        HodlingRecordFor::<T>::insert(key_from, from_hodling);
        HodlingRecordFor::<T>::insert(key_to, to_hodling);
    }
}
...
impl<T: Trait> OnNewSessionForTokenStaking<T::AccountId, T::Balance> for Module<T> {
    fn token_staking_info() -> Vec<(Validator<T::AccountId>, T::Balance)> {
        ...
        syms.into_iter()
            ...
            .map(|sym| {
                let o = OrderPair { first: T::CHAINX_SYMBOL.to_vec(), second: sym.clone() };
                // get price
                let pcx_amount = if let Some(price) = pendingorders::Module::<T>::last_average_price(o) {
                    // get token amount
                    let token_amount: T::TokenBalance = tokenbalances::Module::<T>::total_token(&sym);
                    if price != Zero::zero() {
...
fn change_vote<T: Trait>(who: &T::AccountId, sym: &Symbol, value: T::TokenBalance, is_add: bool) {
    ...
    let mut hodling = Module::<T>::hodling_record_for(&key);
    ...
    VirtualProfsFor::<T>::insert(sym, profs);
    HodlingRecordFor::<T>::insert(key, hodling);
}
