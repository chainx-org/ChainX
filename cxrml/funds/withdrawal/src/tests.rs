...
fn test_check_btc_addr() {
    with_externalities(&mut new_test_ext(), || {
        assert_ok!(financialrecords::Module::<Test>::deposit(&1, &b"btc".to_vec(), 1000));

        let origin = system::RawOrigin::Signed(1).into();
        assert_err!(Module::<Test>::withdraw(origin, b"btc".to_vec(), 100, b"sdfds".to_vec(), b"".to_vec()),
        "verify btc addr err");

        let origin = system::RawOrigin::Signed(1).into();
        assert_ok!(Module::<Test>::withdraw(origin, b"btc".to_vec(), 100,
