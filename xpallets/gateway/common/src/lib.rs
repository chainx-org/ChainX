...
impl<T: Trait> Module<T> {
    ...
    pub fn withdrawal_limit(
        ...
        let info = xpallet_assets_metadata::Module::<T>::asset_info_of(&asset_id)
            .ok_or(xpallet_assets_metadata::Error::<T>::InvalidAsset)?;
    ...
    pub fn verify_withdrawal(
        ...
        let info = xpallet_assets_metadata::Module::<T>::get_asset(&asset_id)?;
        let chain = info.chain();
        match chain {
            Chain::Bitcoin => {
