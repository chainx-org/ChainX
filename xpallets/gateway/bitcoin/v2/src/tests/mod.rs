...
fn test_register_vault() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        assert_err!(
            t_register_vault(alice(), 200_000, "16meyfSoQV6twkAAxPe51RtMVz7PGRmWna"),
...
fn test_add_extra_collateral() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        ...
        assert_err!(
            XGatewayBitcoin::add_extra_collateral(Origin::signed(alice()), 100_000),
            pallet_balances::Error::<Test>::InsufficientBalance
        );
...
fn test_issue_request() {
    ...
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        ...
        let reserved_balance =
            <<Test as xpallet_assets::Config>::Currency>::reserved_balance(bob());
        ...
        assert_eq!(issue_request.requester, bob());
        assert_eq!(issue_request.vault, dave());
...
fn test_cancel_issue_request() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        XGatewayBitcoin::update_issue_griefing_fee(RawOrigin::Root.into(), Percent::from_parts(10))
            .unwrap();
        t_register_vault(dave(), 20000, "16meyfSoQV6twkAAxPe51RtMVz7PGRmWna").unwrap();
        XGatewayBitcoin::request_issue(Origin::signed(alice()), dave(), 1).unwrap();
        ...
        assert_eq!(<pallet::CurrencyOf<Test>>::reserved_balance(dave()), 20000);
        assert_eq!(<pallet::CurrencyOf<Test>>::free_balance(dave()), 10150);
        assert_eq!(<pallet::CurrencyOf<Test>>::free_balance(alice()), 99850);
    })
}
...
fn test_slash_collateral() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        ...
        assert_eq!(<pallet::CurrencyOf<Test>>::free_balance(alice()), 99800);
        assert_eq!(<pallet::CurrencyOf<Test>>::free_balance(bob()), 10200);
    });
}
...
fn test_redeem_request_ok() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        ...
        let vault = XGatewayBitcoin::try_get_vault(&dave()).unwrap();
...
fn test_redeem_execute() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        ...
        assert_eq!(vault.issue_tokens, 0);
    })
}
...
fn test_cancel_redeem_no_reimburse() {
    ExtBuilder::build(BuildConfig::default()).execute_with(|| {
        ...
        let vault_collateral = XGatewayBitcoin::collateral_of(&dave());
