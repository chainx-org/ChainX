...
fn alice<T: Config<I>, I: 'static>() -> T::AccountId {
    // sr25519 Alice
    account::<T, I>("d43593c715fdd31c61141abd04a99fd6822c8558854ccde39a5684e7a56da27d")
}

fn bob<T: Config<I>, I: 'static>() -> T::AccountId {
    // sr25519 Bob
    account::<T, I>("8eaf04151687736326c9fea17e25fc5287613693c912909cb226aa4794f26a48")
}
...
fn prepare_withdrawal<T: Config<I>, I: 'static>() -> Transaction {
    // https://blockchain.info/rawtx/62c389f1974b8a44737d76f92da0f5cd7f6f48d065e7af6ba368298361141270?format=hex
    const RAW_TX: &str = "";
    ...
    let alice = alice::<T, I>();
    let bob = bob::<T, I>();
    let withdrawal_fee = Pallet::<T, I>::btc_withdrawal_fee();
...
fn prepare_headers<T: Config<I>, I: 'static>(caller: &T::AccountId) {
    for (height, header) in generate_blocks_576576_578692() {
        ...
        Pallet::<T, I>::push_header(RawOrigin::Signed(caller.clone()).into(), header).unwrap();
    }
}
...
benchmarks_instance_pallet! {
    ...
    verify {
        assert!(Pallet::<T, _>::headers(&hash).is_some());
    }
    ...
    verify {
        assert!(WithdrawalProposal::<T, I>::get().is_none());
        assert_eq!(
            TxState::<T, I>::get(tx_hash),
            Some(BtcTxState {
    ...
    verify {
        assert!(WithdrawalProposal::<T, I>::get().is_some());
    }
    ...
    verify {
        assert_eq!(WithdrawalProposal::<T, I>::get().unwrap().sig_state, VoteResult::Finish);
    }
    ...
    verify {
        assert_eq!(Pallet::<T, I>::best_index(), best);
    }
    ...
    verify {
        assert_eq!(Pallet::<T, I>::confirmed_index(), Some(confirmed));
    }
    ...
    remove_pending {
        ...
        PendingDeposits::<T, I>::insert(&addr, v);
        let receiver: T::AccountId = whitelisted_caller();
    }: _(RawOrigin::Root, addr.clone(), Some(receiver.clone()))
    verify {
        assert!(Pallet::<T, I>::pending_deposits(&addr).is_empty());
    ...
    force_replace_proposal_tx {
        ...
        Verifier::<T, I>::put(BtcTxVerifier::Test);
        let tx = prepare_withdrawal::<T, I>();
        let raw = serialization::serialize(&tx);
    }: _(RawOrigin::Root, raw.into())
    verify {
        assert_eq!(WithdrawalProposal::<T, I>::get().unwrap().tx, tx);
    }
