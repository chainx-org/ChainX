...
pub fn test_check_trustee_entity() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(
            XGatewayBitcoin::check_trustee_entity(&addr_ok_3).unwrap().0,
            public3
        );
        ...
        assert_eq!(
            XGatewayBitcoin::check_trustee_entity(&addr_ok_2).unwrap().0,
            public2
        );
...
fn test_create_multi_address() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        let hot_info = create_multi_address::<Test, _>(&hot_keys, 3).unwrap();
        let cold_info = create_multi_address::<Test, _>(&cold_keys, 3).unwrap();
...
fn test_verify_signed() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        let result = parse_and_check_signed_tx_impl::<Test, _>(&full_sig_tx, script);
...
fn test_verify_tx_sign() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        for (tx_hex, expect) in cases {
            ...
            let got = parse_and_check_signed_tx_impl::<Test, _>(&tx, script);
...
fn force_replace_withdraw() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        Verifier::<Test, _>::put(BtcTxVerifier::Test);
        ...
        WithdrawalProposal::<Test, _>::put(proposal);
