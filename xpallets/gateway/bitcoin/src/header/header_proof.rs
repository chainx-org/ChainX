...
// Substrate
use frame_support::{debug, dispatch::DispatchResult, traits::UnixTime};
    ...
    fn check<T: Trait>(&self, p: &BtcParams) -> DispatchResult {
        ...
        if work != self.info.header.bits {
            debug::error!(
                ...
                "[check header|work] nBits do not match difficulty rules, \
    ...
    fn check<T: Trait>(&self, p: &BtcParams) -> DispatchResult {
        if let Some(current_time) = self.current_time {
            if self.header.time > current_time + p.block_max_future() {
                debug::error!(
                    target: "xgateway-bitcoin",
                    "[check header|timestamp] header time:{}, current time:{}, max_future{:?}",
                    self.header.time, current_time, p.block_max_future()
                );
            ...
            debug::warn!(
                target: "xgateway-bitcoin",
                "[check header|timestamp] header:{:?}, get unix timestamp error, ignore it",
                self.header.hash()
            );
            Ok(())
        }
