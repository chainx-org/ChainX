...
use super::error::Secp256k1Error;
...
impl<'a> Decoder<'a> {
    ...
    pub fn read(&mut self) -> Result<u8, Secp256k1Error> {
    ...
    pub fn peek(&self, forward: usize) -> Result<u8, Secp256k1Error> {
    ...
    pub fn peek_slice(&self, len: usize) -> Result<&[u8], Secp256k1Error> {
    ...
    pub fn skip(&mut self, len: usize) -> Result<(), Secp256k1Error> {
        if (len == 0 && self.1 >= self.0.len()) || self.1 + len > self.0.len() {
            Err(Secp256k1Error::InvalidSignature)
        } else {
            self.1 += len;
    ...
    pub fn read_constructed_sequence(&mut self) -> Result<(), Secp256k1Error> {
        let v = self.read()?;
        if v == 0x30 {
            Ok(())
        } else {
    ...
    pub fn read_len(&mut self) -> Result<usize, Secp256k1Error> {
        let b1 = self.read()?;
        if b1 == 0xff {
    ...
    pub fn read_integer(&mut self) -> Result<Scalar, Secp256k1Error> {
        if self.read()? != 0x02 {
            return Err(Secp256k1Error::InvalidSignature);
        }
    ...
    pub fn read_seq_len_lax(&mut self) -> Result<usize, Secp256k1Error> {
    ...
    pub fn read_len_lax(&mut self) -> Result<usize, Secp256k1Error> {
        let mut ret = 0usize;
        let mut len = self.read()?;
        if len & 0x80 != 0x00 {
            len -= 0x80;
    ...
    pub fn read_integer_lax(&mut self) -> Result<Scalar, Secp256k1Error> {
        // Integer tag byte.
        if self.read()? != 0x02 {
            return Err(Secp256k1Error::InvalidSignature);
        }
