...
use crate::{
    ...
    BalanceOf, CodeHash, ContractInfo as AliveContractInfo, ContractInfoOf, Trait, TrieId,
};
...
pub fn write_contract_storage<T: Trait>(
    ...
    let mut new_info = match <ContractInfoOf<T>>::get(account) {
        Some(alive) => alive,
        None => return Err(ContractAbsentError),
    };
    ...
    <ContractInfoOf<T>>::insert(&account, new_info);
