...
use xpallet_contracts_primitives::{ContractAccessError, Selector, XRC20Selector};
use xpallet_support::{debug, ensure_with_errorlog, error, try_hex};
...
decl_module! {
    ...
    pub struct Module<T: Trait> for enum Call where origin: <T as frame_system::Trait>::Origin {
        ...
        #[weight = 0]
        pub fn set_println(origin, state: bool) -> DispatchResult {
            ensure_root(origin)?;
            CurrentSchedule::mutate(|s| {
...
impl<T: Trait> Module<T> {
    ...
    pub fn call_xrc20(id: AssetId, selector: XRC20Selector, data: Vec<u8>) -> ExecResult {
        match selector {
            XRC20Selector::Issue | XRC20Selector::Destroy => {
                return Err(ExecError {
                    reason: Error::<T>::InvaliedXRC20Selector.into(),
                    buffer: Vec::new(),
                })
            }
            _ => {}
    ...
    fn issue_to_xrc20(id: AssetId, origin: T::AccountId, value: T::Balance) -> DispatchResult {
        ...
        let result: bool = Decode::decode(&mut exec_value.data.as_slice()).map_err(|e| {
            error!(
                "[issue_to_xrc20]|fail to decode wasm result|e:{:?}|data:{:?}",
                e,
                try_hex!(exec_value.data)
            );
            Error::<T>::DecodeErr
        })?;
        if !result {
    ...
    fn refund_to_asset(
        ...
        let current_reserved = xpallet_assets::Module::<T>::asset_balance_of(
            &contract_addr,
            &id,
            AssetType::ReservedXRC20,
        );
