...
fn call<R>(client: &TClient, at: &BlockId, function: &'static str, input: &[u8]) -> Result<R>
where
    R: Decode,
{
    let parent = at;
    let header = Header {
        parent_hash: client.block_hash_from_id(&parent)?.ok_or_else(|| {
            ErrorKind::UnknownBlock(format!("{:?}", parent))
        })?,
        number: client.block_number_from_id(&parent)?.ok_or_else(|| {
            ErrorKind::UnknownBlock(format!("{:?}", parent))
        })? + 1,
    ...
    client.state_at(&parent).map_err(Error::from).and_then(
        |state| {
            ...
            Ok(Decode::decode(&mut &r[..]).ok_or_else(|| {
                client::error::Error::from(client::error::ErrorKind::CallResultDecode(function))
            })?)
        },
    )
}
...
impl ChainXApi for TClient {
    ...
    fn index(&self, at: &BlockId, account: AccountId) -> Result<Index> {
        account.using_encoded(|encoded| call(self, at, "account_nonce", encoded))
    }

    fn lookup(&self, at: &BlockId, address: Address) -> Result<Option<AccountId>> {
        address.using_encoded(|encoded| call(self, at, "lookup_address", encoded))
    }
    ...
    fn inherent_extrinsics(
        ...
        inherent_data
            .using_encoded(|encoded| call(self, at, "inherent_extrinsics", encoded))
    }
}
