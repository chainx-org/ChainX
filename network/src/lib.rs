...
impl ChainXProtocol {
	...
	fn on_chainx_message(&mut self, ctx: &mut Context<Block>, who: NodeIndex, _raw: Vec<u8>, msg: Message) {
			...
			generic_message::Message::ChainSpecific(raw) => {
				match Message::decode(&mut raw.as_slice()) {
					...
					None => {
						...
						ctx.report_peer(who, Severity::Bad("Invalid chainx protocol message format"));
					}
				}
			}
			_ => {}
