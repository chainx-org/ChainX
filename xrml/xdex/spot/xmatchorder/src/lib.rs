...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn on_finalise(time: T::BlockNumber) {
            //先读取xpendingorders模块的所有新挂单

            let max_command_id: u64 = <xpendingorders::Module<T>>::max_command_id();

            for command_id in 1..(max_command_id + 1) {
                if let Some(command) = <xpendingorders::Module<T>>::command_of(command_id) {
                    if let Some(order) = <xpendingorders::Module<T>>::order_of((
                        command.0.clone(),
                        command.1.clone(),
                        command.2,
                    )) {
                        if command.3 == CommandType::Cancel {
                            ...
                            <xpendingorders::Module<T>>::update_command_of(command_id, cancel_bid);
                            // 记录日志
                            ...
                            <xpendingorders::Module<T>>::update_command_of(
                                command_id,
                                new_last_bid_index,
                            );
                            // 记录日志
            ...
            //清空
            <xpendingorders::Module<T>>::clear_command_and_put_fee_buy_order();
        }
...
decl_event!(
    pub enum Event<T> where
        ...
        <T as xpendingorders::Trait>::Amount,
        <T as xpendingorders::Trait>::Price
    {
        SetMatchFee(Amount),
...
pub type BidT<T> = Bid<<T as xpendingorders::Trait>::Amount, <T as xpendingorders::Trait>::Price>;
...
impl<T: Trait> Module<T> {
                    ...
                    for kk in 0..node.data.list.len() {
                        if let Some(mut match_bid) = Self::bid_of(node.data.list[kk]) {
                            ...
                            let taker_fee: T::Amount = Self::calculation_taker_fee(amount); // 手续费
                                                                                            //成交
                            if let Err(_msg) = <xpendingorders::Module<T>>::fill_order(
    ...
    fn remove_from_bid_list(
        node: &mut Node<
            Bid<<T as xpendingorders::Trait>::Amount, <T as xpendingorders::Trait>::Price>,
        >,
        remove_id: &Vec<BidId>,
    ) {
