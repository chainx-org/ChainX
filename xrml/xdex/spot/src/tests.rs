...
fn test_order() {
    with_externalities(&mut new_test_ext(0, 3, 3, 0, true, 10), || {
        ...
        assert_eq!(Spot::put_order(Some(a).into(),0,OrderType::Limit,OrderDirection::Buy,1000,100000),Ok(()));
        assert_eq!(Assets::asset_balance(&a, &BTC,Free), 1_000_000_000-(1000*100000));
        assert_eq!(Assets::asset_balance(&a, &BTC,ReservedDexSpot), 1000*100000);
        assert_eq!(Assets::asset_balance(&a,&PCX,Free), 1_000_000_000);
        assert_eq!(Assets::asset_balance(&a, &PCX,ReservedDexSpot), 0);

        //a 第二笔挂买单 1000*50
        assert_eq!(Spot::put_order(Some(a).into(),0,OrderType::Limit,OrderDirection::Buy,1000,50),Ok(()));
        assert_eq!(Assets::asset_balance(&a, &BTC,Free), 1_000_000_000-(1000*100000)-(1000*50));
        assert_eq!(Assets::asset_balance(&a, &BTC,ReservedDexSpot), 1000*100000+1000*50);
        assert_eq!(Assets::asset_balance(&a,&PCX,Free), 1_000_000_000);
        assert_eq!(Assets::asset_balance(&a, &PCX,ReservedDexSpot), 0);
        ...
        assert_eq!(Spot::put_order(Some(b).into(),0,OrderType::Limit,OrderDirection::Sell,1000,200000),Ok(()));
        assert_eq!(Assets::asset_balance(&b, &BTC,Free), 1_000_000_000);
        assert_eq!(Assets::asset_balance(&b, &BTC,ReservedDexSpot), 0);
        ...
        //b 第二笔挂卖单 500*100000
        assert_eq!(Spot::put_order(Some(b).into(),0,OrderType::Limit,OrderDirection::Sell,500,100000),Ok(()));
        ...
        assert_eq!(Assets::asset_balance(&b, &BTC,Free), 1_000_000_000+500*100000);
        assert_eq!(Assets::asset_balance(&b, &BTC,ReservedDexSpot), 0);
        assert_eq!(Assets::asset_balance(&b,&PCX,Free), 1_000_000_000-1000-500);
        assert_eq!(Assets::asset_balance(&b, &PCX,ReservedDexSpot), 1000);
        ...
        assert_eq!(Assets::asset_balance(&a, &BTC,Free), 1_000_000_000-(1000*100000)-(1000*50)+500*100000);
        assert_eq!(Assets::asset_balance(&a, &BTC,ReservedDexSpot), 1000*100000+1000*50-500*100000-500*100000);
        assert_eq!(Assets::asset_balance(&a,&PCX,Free), 1_000_000_000+500);
        assert_eq!(Assets::asset_balance(&a, &PCX,ReservedDexSpot), 0);
        ...
        //a 第三笔挂买单 
        assert_eq!(Spot::put_order(Some(a).into(),0,OrderType::Limit,OrderDirection::Buy,1000,100),Ok(()));
        assert_eq!(Assets::asset_balance(&a, &BTC,Free), 1_000_000_000-(1000*100000)-(1000*50)+500*100000-1000*100);
        assert_eq!(Assets::asset_balance(&a, &BTC,ReservedDexSpot), 1000*100000+1000*50-500*100000-500*100000+1000*100);
        assert_eq!(Assets::asset_balance(&a,&PCX,Free), 1_000_000_000+500);
        assert_eq!(Assets::asset_balance(&a, &PCX,ReservedDexSpot), 0);
