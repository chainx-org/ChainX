...
use super::{BalanceOf, CodeHash, ContractInfo, ContractInfoOf, Trait, TrieId, TrieIdGenerator};
    ...
    fn commit(&mut self, s: ChangeSet<T>) {
        ...
        for (address, changed) in s.into_iter() {
                ...
                || changed.reset
            {
                ...
                // };
                let old_info = <ContractInfoOf<T>>::get(&address);
                let mut new_info = match (changed.reset, old_info.clone(), changed.code_hash) {
                    ...
                    (_, None, Some(code_hash)) => ContractInfo::<T> {
                        code_hash,
                        storage_size: 0,
                        empty_pair_count: 0,
                        total_pair_count: 0,
                ...
                for (key, opt_new_value) in changed.storage.into_iter() {
                    ...
                    match opt_new_value {
                        Some(new_value) => {
                            child::put_raw(&child_trie_info, &hashed_key, &new_value[..])
                        }
                    ...
                    .unwrap_or(true)
                {
                    ...
                    <ContractInfoOf<T>>::insert(&address, new_info);
                }
            }
        }
