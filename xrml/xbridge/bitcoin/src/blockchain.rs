...
use {
    BestIndex, BlockHeaderFor, CertCache, DepositCache, DepositInfo, HashsForNumber, Module,
...
impl<T: Trait> Chain<T> {
    ...
    fn canonize(hash: &H256) -> Result<(), ChainErr> {
        ...
        if let Some(vec) = <DepositCache<T>>::take() {
            ...
            let mut uncomplete_cache: Vec<DepositInfo<T::AccountId>> = Vec::new();
            //for (account_id, amount, tx_hash, block_hash) in vec {
            for deposit_info in vec {
                match <NumberForHash<T>>::get(deposit_info.block_hash.clone()) {
                    Some(height) => {
                        if new_best_header.number >= height + irr_block {
                            runtime_io::print(deposit_info.account.encode().as_slice());
                            runtime_io::print(deposit_info.btc_balance);
                            ...
                            runtime_io::print(new_best_header.number as u64);
                            runtime_io::print(height as u64);
                            uncomplete_cache.push(deposit_info);
                        }
                    }
                    None => {
