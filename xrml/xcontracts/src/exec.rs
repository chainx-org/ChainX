...
pub trait Ext {
    ...
    fn instantiate(
        ...
        value: <Self::T as xassets::Trait>::Balance,
    ...
    fn note_restore_to(
        ...
        rent_allowance: <Self::T as xassets::Trait>::Balance,
    ...
    fn balance(&self) -> <Self::T as xassets::Trait>::Balance;

    /// Returns the value transferred along with this call or as endowment.
    fn value_transferred(&self) -> <Self::T as xassets::Trait>::Balance;

    /// Returns a reference to the timestamp of the current block
    fn now(&self) -> &MomentOf<Self::T>;

    /// Returns the minimum balance that is required for creating an account.
    fn minimum_balance(&self) -> <Self::T as xassets::Trait>::Balance;
    ...
    fn set_rent_allowance(&mut self, rent_allowance: <Self::T as xassets::Trait>::Balance);

    /// Rent allowance of the contract
    fn rent_allowance(&self) -> <Self::T as xassets::Trait>::Balance;
    ...
    V: Vm<T, Executable = E>,
{
    ...
    pub fn call(
        ...
        let contract_info = <ContractInfoOf<T>>::get(&dest);
        match contract_info {
            Some(ContractInfo::Tombstone(_)) => {
                return Err(ExecError {
                    reason: "not allow Tombstone contract",
                    buffer: input_data,
                })
            }
            _ => {}
        ...
        // Calls to dead contract always fail.
    ...
    pub fn terminate(
        ...
        if let Some(caller) = self.caller {
            if caller.is_live(&self_id) {
                return Err("Cannot terminate a contract that is present on the call stack");
            }
        }
        transfer(
            gas_meter,
    ...
    fn is_live(&self, account: &T::AccountId) -> bool {
        &self.self_account == account || self.caller.map_or(false, |parent| parent.is_live(account))
    }
}
    ...
    L: Loader<T, Executable = E>,
{
    ...
    fn set_rent_allowance(&mut self, rent_allowance: T::Balance) {
        self.ctx
            .overlay
    ...
    fn rent_allowance(&self) -> T::Balance {
        self.ctx
            ...
            .unwrap_or(<T::Balance>::max_value()) // Must never be triggered actually
    }
...
mod tests {
    use super::{
        DeferredAction, ExecFeeToken, ExecResult, ExecutionContext, Ext, Loader, RawEvent,
        TransferFeeKind, TransferFeeToken, Vm,
    };
    use crate::account_db::AccountDb;
    use crate::exec::{ExecError, ExecReturnValue, STATUS_SUCCESS};
