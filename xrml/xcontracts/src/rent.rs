...
use crate::{
    AliveContractInfo, ContractInfo, ContractInfoOf, Module, TombstoneContractInfo, Trait,
};
...
fn try_evict_or_and_pay_rent<T: Trait>(
    ...
    let fee_per_block = {
        ...
        let effective_storage_size =
            <T::Balance>::from(contract.storage_size).saturating_sub(free_storage);
    ...
    let subsistence_threshold = T::TombstoneDeposit::get();
    ...
    let can_withdraw_rent = dues_limited < xassets::Module::<T>::pcx_free_balance(account);

    if can_withdraw_rent && (insufficient_rent || pay_rent) {
        // Collect dues.
        Module::<T>::transfer_to_council(account, dues_limited);
    }
