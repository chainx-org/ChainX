...
use super::{
    AliveContractInfo, CodeHash, ContractInfo, ContractInfoOf, Trait, TrieId, TrieIdGenerator,
};
...
use support::traits::{Get, Imbalance, SignedImbalance};
...
pub struct ChangeEntry<T: Trait> {
    ...
    rent_allowance: Option<T::Balance>,
...
impl<T: Trait> ChangeEntry<T> {
    fn balance(&self) -> Option<T::Balance> {
    ...
    fn rent_allowance(&self) -> Option<Option<T::Balance>> {
        if self.reset {
...
pub trait AccountDb<T: Trait> {
    ...
    fn get_rent_allowance(&self, account: &T::AccountId) -> Option<T::Balance>;
...
impl<T: Trait> AccountDb<T> for DirectAccountDb {
    ...
    fn get_balance(&self, account: &T::AccountId) -> T::Balance {
        xassets::Module::<T>::pcx_free_balance(account)
    }
    fn commit(&mut self, s: ChangeSet<T>) {
        let mut total_imbalance = SignedImbalance::zero();
        for (address, changed) in s.into_iter() {
            if let Some(balance) = changed.balance() {
                let imbalance = xassets::Module::<T>::pcx_make_free_balance_be(&address, balance);
                ...
                let mut new_info = match (changed.reset, old_info.clone(), changed.code_hash) {
                    ...
                    (true, Some(info), Some(code_hash)) => {
                        ...
                        AliveContractInfo::<T> {
                            ...
                            rent_allowance: <T::Balance>::max_value(),
                            last_write: None,
                        }
                    }
...
impl<'a, T: Trait> OverlayAccountDb<'a, T> {
    ...
    pub fn instantiate_contract(
        ...
        contract.rent_allowance = Some(<T::Balance>::max_value());
    ...
    pub fn set_rent_allowance(&mut self, account: &T::AccountId, rent_allowance: T::Balance) {
        self.local
            .borrow_mut()
            .entry(account.clone())
            .or_insert(Default::default())
